...
const ALLOWED_ITEMS = [
  ...
  "Signature Pads","Calculator","Tea Cups","Coffee Cups","Water Bottles","Jugs","trees", "plants","Flasks","Plates","Business Cards","Letterheads","Posters","Banners","Hoardings",
...
const showToast = (msg = 'Copied!', type = 'info') => {
  ...
  // Add a subtle vibration effect on error toasts
  if (type === 'error') {
...
const generateRemarks = (result) => {
  ...
  // Add special note for handwritten content
  if (result.handwrittenNotes && result.handwrittenNotes.length > 0) {
    remarks.recommendations.push("Handwritten content detected - please verify accuracy");
    // Add specific recommendation for handwritten content
    remarks.recommendations.push("Handwritten items may require additional verification for accuracy");
    // Add warning about potential OCR challenges with handwritten content
    remarks.warnings.push("Handwritten content may have OCR recognition challenges");
  }
...
const downloadCSV = (lastValidationResult) => {
  const csvContent = generateCSV(lastValidationResult);
  const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = 'bill_items.csv';
...
['dragenter','dragover'].forEach(ev => {
  dropZone.addEventListener(ev, () => {
    ...
    // Add a subtle scale effect when dragging over
...
document.addEventListener('paste', e => {
  ...
  if(item){
    ...
    // Add a subtle animation to the drop zone when pasting
    dropZone.style.animation = 'pulse 0.5s';
...
async function validateBill() {
  ...
  setStep(1);
  updateProgress(25);
  // Provide specific status message for PDF files
  if (currentFile.type === 'application/pdf') {
    statusText.textContent = 'Encoding PDF…';
  } else {
  ...
  try {
    ...
    if (currentFile && currentFile.type === 'application/pdf') {
      showError('Could not read PDF file. Please ensure it\'s not corrupted and try a different file.');
    } else {
  ...
  // Provide specific status message for PDF files
  if (currentFile.type === 'application/pdf') {
    statusText.textContent = 'Analyzing PDF with AI…';
  } else {
    statusText.textContent = 'Analyzing with AI…';
  }

  try {
    // Set appropriate MIME type for the API
    let mimeType = currentFile.type || 'image/jpeg';
    // For PDF files, ensure we use the correct MIME type
    ...
    if (!resultObj.isItemValid && !resultObj.isCalculationValid && 
      ...
      // Provide specific error message for PDF files
      if (currentFile && currentFile.type === 'application/pdf') {
        showError('Invalid bill: The uploaded PDF is not a valid bill/receipt. Please upload a proper bill PDF.');
      } else {
...
function handleFiles(){
  ...
  if (fileType === 'application/pdf') {
    ...
    thumb.alt = 'PDF File';
    thumb.src = 'data:image/svg+xml;base64,';
    
    // Update timeline steps for PDF files
    document.querySelector('#st1 div h4').textContent = 'Upload';
    document.querySelector('#st1 div p').textContent = 'PDF received & prepared.';
    document.querySelector('#st2 div h4').textContent = 'Analyze';
    document.querySelector('#st2 div p').textContent = 'Sending PDF to AI for extraction.';
  } else {
    ...
    thumb.onerror = () => {
      ...
      showError('Failed to load preview. Please try another file.');
    };
    
    // Reset timeline steps for image files
    document.querySelector('#st1 div h4').textContent = 'Upload';
    document.querySelector('#st1 div p').textContent = 'Image received & prepared.';
    document.querySelector('#st2 div h4').textContent = 'Analyze';
    document.querySelector('#st2 div p').textContent = 'Sending to AI for extraction.';
  }
  ...
  const displayFileType = f.type || 'image/*';
  const fileTypeEl = document.getElementById('fileType');
  if (fileTypeEl) {
    fileTypeEl.textContent = displayFileType;
...
zoomBtn.addEventListener('click', () => {
  if(thumb.src && currentFile){
    // Create a modal for better file viewing
    ...
    // Handle PDF files differently from images
    if (currentFile.type === 'application/pdf') {
      // For PDF files, create an embed element to display the actual PDF content
      const pdfEmbed = document.createElement('embed');
      pdfEmbed.src = URL.createObjectURL(currentFile);
    ...
    closeBtn.onclick = () => {
      // Revoke the object URL to free memory when closing the modal
      if (currentFile.type === 'application/pdf') {
        const pdfEmbed = container.querySelector('embed');
...
function showResults(r){
  ...
  const existingComprehensiveBillInfo = document.querySelector('.comprehensive-bill-info');
  if (existingComprehensiveBillInfo) {
    existingComprehensiveBillInfo.remove();
  }
  ...
  // Remove any existing validation summary to prevent duplication
  const existingValidationSummary = document.querySelector('.validation-summary');
  if (existingValidationSummary) {
    existingValidationSummary.remove();
  }
...
// Initialize theme
// updateTheme(); // Removed duplicate call
